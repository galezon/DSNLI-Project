...
![postcode_risk_img](../postcode_risk.png)

## Finding bins for *AGEPH*.
...
Now, we just need to repeat the whole process, but for severity. However, we discussed in class that we condition severity on having at least one claim. This subsetting below reflects that.
```{r}
sev_data = data[data[, "nbrtotc"] > 0, ]
nrow(sev_data)
```
We now need to fit in a GAM.
