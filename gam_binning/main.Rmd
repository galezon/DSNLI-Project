...
nrow(sev_data)
```
We now need to fit in a GAM. Note that the model I ended up with is *log(chargtot) = log(expo) + agecar + sexp + fuelc + split + fleetc + sportc + powerc + S(LAT, LONG) + s(AGEPH)*. The final GAM model is called *sev_gam3*.
...
```{r}
sev_f1 = chargtot ~ offset(log(duree)) + agecar + sexp + fuelc + split + usec + fleetc + sportc + coverp + powerc + s(LAT, LONG, bs="tp") + s(AGEPH, bs="cr")
sev_gam1 = gam(sev_f1, Gamma(link="log"), data=sev_data)
summary(sev_gam1)
print("------------------------------------------------------------------")
anova(sev_gam1)
print("------------------------------------------------------------------")
...
Let us drop those candidates. Smooth effects definitely need to be kept.
```{r}
sev_f2 = update(sev_f1, ~. - coverp - usec - fleetc - sportc - powerc )
sev_gam2 = gam(sev_f2, Gamma(link="log"), data=sev_data)
summary(sev_gam2)
print("------------------------------------------------------------------")
anova(sev_gam2)
par(mfrow=c(1, 2))
...
A quick BIC check. Looks like we dropped too many at once. However, since we only have 11k datapoints, the training of a single GAM model is very quick, so I think we can try stepwise dropping.
```{r}
print(paste("BIC(sev_gam1) =", BIC(sev_gam1)))
print(paste("BIC(sev_gam2) =", BIC(sev_gam2)))
```

Trying out different variables to drop.
```{r}
nocoverp_formula = update(sev_f1, ~. - coverp)
nousec_formula = update(sev_f1, ~. - usec)
nofleetc_formula = update(sev_f1, ~. - fleetc)
nosportc_formula = update(sev_f1, ~. - sportc)
nopowerc_formula = update(sev_f1, ~. - powerc)

sev_fam = Gamma(link="log")
nocoverp_gam = gam(nocoverp_formula, sev_fam, sev_data)
nousec_gam = gam(nousec_formula, sev_fam, sev_data)
nofleetc_gam = gam(nofleetc_formula, sev_fam, sev_data)
nosportc_gam = gam(nosportc_formula, sev_fam, sev_data)
nopowerc_gam = gam(nopowerc_formula, sev_fam, sev_data)
```


BIC(sev_gam1) is 234900, so *coverp* and *usec* are candidates to drop.
```{r}
print(paste("BIC(nocoverp_gam) =", BIC(nocoverp_gam)))
print(paste("BIC(nousec_gam) =", BIC(nousec_gam)))
print(paste("BIC(nofleetc_gam) =", BIC(nofleetc_gam)))
print(paste("BIC(nosportc_gam) =", BIC(nosportc_gam)))
print(paste("BIC(nopowerc_gam) =", BIC(nopowerc_gam)))
```

Let us try dropping *usec* and *coverp*. I tried dropping more variables, but I couldn't find anything better in terms of BIC. The coefficients below are pretty significant, including the smooth effects.
```{r, warning=FALSE}
sev_f3 = update(sev_f1, ~. - usec - coverp)
sev_gam3 = gam(sev_f3, sev_fam, data=sev_data)
summary(sev_gam3)
print("------------------------------------------------------------------")
anova(sev_gam3)
print("------------------------------------------------------------------")
print(paste("BIC(nocoverp_gam) =", BIC(nocoverp_gam)))
print(paste("BIC(nousec_gam) =", BIC(nousec_gam)))
print(paste("BIC(sev_gam3) =", BIC(sev_gam3)))
par(mfrow=c(1, 2))
plot(sev_gam3, scheme=2)
```
...
postcodes['powerc'] = data[1, 'powerc']
sev_preds = predict.gam(sev_gam3, newdata=postcodes, type="terms", terms=s(LAT, LONG))
...
```{r}
sev_base_formula = update(sev_f3, ~. -S(LAT, LONG, bs="tp") )  # no longer needed
...
Even with BIC, the best is 19 clusters. I'm not willing to work with that many clusters, so I think I will limit the search into 10 clusters only. In reflection I should've realized this as well for the frequency gathering (imposing a strict "I want x many clusters maximum"), but alas, I'd rather not go back there.
...
```{r, echo=FALSE}
sev_postcode_to_cluster = sev_fctr_rslts[, c("CODPOSS", "x4clusters")]
saveRDS(sev_postcode_to_cluster, file="../data/sev_postcode_bins.Rda")

barplot(summary(as.factor(sev_postcode_to_cluster[, "x4clusters"])))
```
...
sev_age_dat['powerc'] = data[1, 'powerc']
sev_age_dat['LAT'] = 1
sev_age_dat['LONG'] = 1

preds = as.data.frame(predict.gam(sev_gam3, sev_age_dat, type="terms", terms=s(AGEPH)))
...
sev_tmp = sev_tmp[, c("agecar", "sexp", "fuelc", "split", "fleetc", "sportc", "powerc",
              "x4clusters", "duree", "AGEPH", "chargtot")]
```


Trying evtree binning on a large grid. Looks like *alpha* values between 15-45, *depth*=4 would be ideal, with the exact best being 25. Since this part of the grid is only a jump of 5 each, I won't re-partition the grid any further. Thus, I'll set *alpha* as 25.
```{r}
sev_glm_formula = chargtot~ agecar + sexp + fuelc + split + fleetc + sportc + powerc + x4clusters + AGEPH_binned + offset(log(duree))
...
ctrl = evtree.control(minbucket=0.05*nrow(sev_age_dat), maxdepth=4, alpha=25,
                      niterations=20000, ntrees=200)
...
sev_age_df = smooth_to_factors(age_df, splits, on_var="AGEPH")
